...
const DoseCalculatorScreen = () => {
  ...
  const [selectedWeight, setSelectedWeigth] = useState<WeightUnits>(
    WeightUnits.KG,
  );
  ...
  const calculateDose = () => {
    ...
    if (selectedDrug?.type === DrugType.TYLENOL) {
      ...
      selectedDrug?.type === DrugType.ADVIL &&
      convertedAge >= 1 &&
      convertedAge < 6
    ) {
            ...
            >
              <VStack>
                <HStack space="md" alignItems="center">
                  ...
                  <ToastTitle bold fontSize="$lg" color="$red600">
                    {t("defaults.errorTitle")}
                  </ToastTitle>
                </HStack>
      ...
      <View>
        ...
        <Center gap={10} px="$5">
          ...
          {selectedDrug && (
            <>
              <Text fontSize="$lg">
                {t(`drugsData.${selectedDrug.id}.description`)}
              </Text>
              ...
              <DropDown
                ...
                data={[
                  { label: "kg", value: WeightUnits.KG.toString() },
                  { label: "lb", value: WeightUnits.LB.toString() },
                ]}
              ...
              {!selectedDrug.requireAge ? null : (
                <>
                  ...
                  <DropDown
                    ...
                    data={[
                      { label: "Month(s)", value: AgeUnits.MONTH.toString() },
                      { label: "Year(s)", value: AgeUnits.YEAR.toString() },
                    ]}
              ...
              <Button onPress={calculateDose} bg="$fuchsia700" p="$2">
                ...
                <ButtonText fontSize="$2xl" pl="$3">
                  {t("calculator.calulateBtn")}
                </ButtonText>
              </Button>
              {calculatedDose ? (
                <PopUpDialog
                  ...
                  body={
                    <Box>
                      <VStack space="md">
                        <WarningMessage
                          text={t("calculator.dilaog.bodyWaringBody")}
                        />
                        <Center
                        ...
                        >
                          <Text fontSize="$7xl" color="$secondary800">
                            {t(
                              `drugsData.${selectedDrug.id}.calculatedDoseUnit`,
                              {
                                dose: (
                                  Math.round(
                                    (calculatedDose + Number.EPSILON) * 100,
                                  ) / 100
                                ).toFixed(1),
                              },
                            )}
